...
equation eq12(tu,c); eq12(tu,c) .. sum(tup, F_rec(tup,tu) * C_rout(tup,tu,c)) + sum(s, F_s_tu(s,tu) * C_c_s(s,c)) =e= ML_in(tu,c) ;
equation eq13(tu,c); eq13(tu,c) .. C_out(tu,c) * ( sum(tup, F_rec(tu,tup)) + F_exit(tu) ) + dummy(tu,c) =e= ML_out(tu,c) ;
